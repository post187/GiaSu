...
// 1. Cập nhật Type đúng với dữ liệu Backend mới trả về
type CalendarSession = {
  id: string;
  classId: string;
  classTitle: string;
  tutorName: string; // Backend trả về tên Gia sư cho học sinh
  subjectName: string;
...
export default function StudentCalendarPage() {
  ...
  const t = {
    ...
    empty: language === "vi" ? "Chưa có lớp học nào" : "No classes scheduled",
    refresh: language === "vi" ? "Tải lại" : "Refresh",
    startBtn: language === "vi" ? "Vào lớp" : "Start",
    completeBtn: language === "vi" ? "Hoàn thành" : "Complete",
  };
  ...
  // Helper: Tính toán ngày giờ thực tế của buổi học TRONG TUẦN NÀY
  // Để dùng cho logic check nút Bắt đầu / Hoàn thành
  const calculateSessionDateTime = (dayOfWeek: number, timeStr: string) => {
    const now = new Date();
  ...
  // Logic check nút Bắt đầu (cho phép vào trước 15p và trong vòng 60p sau khi bắt đầu)
  const canStart = (s: CalendarSession) => {
    const start = calculateSessionDateTime(s.dayOfWeek, s.startTime).getTime();
    const now = Date.now();

    // Logic: Chỉ hiện nút nếu hôm nay ĐÚNG là ngày học
  ...
  // Logic check nút Hoàn thành (sau khi kết thúc giờ học)
  const canComplete = (s: CalendarSession) => {
    const end = calculateSessionDateTime(s.dayOfWeek, s.endTime).getTime();
    const now = Date.now();
    ...
    <DashboardLayout requiredRole={["STUDENT"]}>
      <div className="p-8 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={() => mutate()}
            className="px-4 py-2 rounded-lg text-sm font-medium bg-gradient-to-r from-purple-500 to-pink-500 text-white shadow hover:opacity-90 transition-opacity"
          >
            {t.refresh}
          </button>
        </div>
              ...
              >
                ...
                <div
                  className={`absolute left-0 top-0 bottom-0 w-1 ${
                    s.status === "TRIAL" ? "bg-amber-400" : "bg-green-500"
                  }`}
                />

                <div className="flex flex-col md:flex-row md:items-center gap-6 pl-2">
                  {/* Cột 1: Thời gian */}
                  <div className="flex flex-col items-center justify-center min-w-[120px] text-center md:border-r border-slate-200 dark:border-slate-700 md:pr-6">
                    <span
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-slate-500 dark:text-slate-400">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span>
                          {t.tutor}:{" "}
                          <span className={textColor}>{s.tutorName}</span>
                        </span>
                      </div>
                      {s.subjectName && (
                        <div className="flex items-center gap-1.5">
